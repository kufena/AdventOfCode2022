...
/*
int[] floorheights = new int[7] { 0, 0, 0, 0, 0, 0, 0 };
...
while (count > 0)
{
    ...
    for (int j = posx; j < posx + shape.width; j++)
    {
...
bool Landed(Shapes shape, int[] floor, int posx, int posy)
{
    for (int i = posx; i < posx + shape.width; i++)
    {
...
bool Covers(Shapes shape, int[] floor, int posx, int posy)
{
    for (int i = posx; i < posx + shape.width; i++)
    {
        if ((posy + shape.botProfile[i - posx]) <= floor[i])
